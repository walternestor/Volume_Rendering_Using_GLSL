...
void main()
{
    ...
    for (int i = 0; i < numSamples && travel > 0.0; ++i, pos += deltaDir, travel -= StepSize, lengthAcum += deltaDirLen)
    {
        ...
        float redMinimunThreshold = redCenter - (redWidth / 2);
        float redMaximunThreshold = redCenter + (redWidth / 2);

        if (intensity <= redMinimunThreshold) {
            colorSample.r = 0.0;
        }
        else if (intensity > redMaximunThreshold){
            colorSample.r = 1.0;
        }
        else {
            colorSample.r = ((intensity - redCenter + (redWidth / 2)) / redWidth);
        ...
        float greenMinimunThreshold = greenCenter - (greenWidth / 2);
        float greenMaximunThreshold = greenCenter + (greenWidth / 2);

        if (intensity <= greenMinimunThreshold) {
            colorSample.g = 0.0;
        }
        else if (intensity > greenMaximunThreshold){
            colorSample.g = 1.0;
        }
        else {
            colorSample.g = ((intensity - greenCenter + (greenWidth / 2)) / greenWidth);
        ...
        float blueMinimunThreshold = blueCenter - (blueWidth / 2);
        float blueMaximunThreshold = blueCenter + (blueWidth / 2);

        if (intensity <= blueMinimunThreshold) {
            colorSample.b = 0.0;
        }
        else if (intensity > blueMaximunThreshold){
            colorSample.b = 1.0;
        }
        else {
            colorSample.b = ((intensity - blueCenter + (blueWidth / 2)) / blueWidth);
        ...
        float alphaMinimunThreshold = alphaCenter - (alphaWidth / 2);
        float alphaMaximunThreshold = alphaCenter + (alphaWidth / 2);
